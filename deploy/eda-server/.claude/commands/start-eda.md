...
5. Tell the user the tool is running and they can now run `/synthesis`, `/floorplan`, `/placement`, or other stage commands.
...
- Stage commands (`/synthesis`, `/floorplan`, etc.) will call `eda.start_tool` internally if needed, but `/start-eda` lets the user explicitly start the tool first.
