...
| Element | Manual Mode | Auto Mode |
|---------|-------------|-----------|
| **Mode** | `⚙ HiPilot │ 🔒 Manual` | `⚡ Claude has conn` |
| **Pending** | `⏳` (when Tcl queued) | Hidden |
| **Tool** | Current EDA tool name | Current EDA tool name |
| **Job** | `▶ idle` / `▶ Running` | `▶ idle` / `▶ Running` |
| **Design** | Design name or `—` | Design name or `—` |
| **QoR** | WNS/TNS values | WNS/TNS values |

### 3. TUI Components (React/Ink)
When `hipilot status` or `hipilot skills` is run:
- **Dashboard**: Shows current mode, active tool, job status
...
### 4. What Screenshots Must Show
Every test phase must capture:
- [ ] **Initial state**: Clean two-pane layout after launch
- [ ] **Mode indicator**: Current mode visible in status bar
- [ ] **Command input**: Text being typed in left pane
- [ ] **EDA activity**: Tool output in right pane
- [ ] **Approval state**: Pending Tcl with ⏳ indicator (if applicable)
...
### 1. Kill All Remaining Processes (CRITICAL)
Stale processes cause test failures and false results. Must clean thoroughly:

```bash
# Kill EDA tools (lock database files)
...
# Kill MCP servers and claude
pkill -9 -f "servers/eda/index.js" 2>/dev/null || true
pkill -9 -f "servers/tmux/index.js" 2>/dev/null || true
pkill -9 -f "servers/knowledge/index.js" 2>/dev/null || true
pkill -9 -f "claude --dangerously-skip-permissions" 2>/dev/null || true

# Verify nothing remains
ps aux | grep -E "(innovus|icc2_shell|pt_shell|hipilot|claude|ffmpeg)" | grep -v grep
...
- Press keyboard shortcuts (`prefix+y` to approve)
...
- Press keyboard shortcuts (prefix+y, prefix+m)
...
**Goal**: Tcl generation works, approval workflow functions

### Tests
1. **Tcl Generation (Template)**
   - Prompt: "/timing" or "generate timing report"
   - Verify `[✓ Template]` badge appears
   - Check Tcl saved to pending.tcl (manual mode)

2. **Approval Workflow**
   - Verify pending.tcl exists
   - Press `prefix+y` to approve
   - Verify Tcl executes in right pane
...
4. **Auto Mode Toggle**
   - Press `prefix+m` to enable auto mode
   - Verify safe commands execute immediately
   - Verify dangerous commands still require confirmation

### Success Criteria
- [ ] Tcl generates with proper badge
- [ ] Manual mode queues to pending.tcl
- [ ] Approval executes Tcl
- [ ] Auto mode works for safe commands

### Evidence
- pending.tcl content captured
- screenshot showing approval workflow
- EDA output showing command executed

### Exit Criteria
**PASS**: Approval workflow works → Proceed to Phase 3
**FAIL**: Fix mode/approval system
...
| **Status Bar** | Mode indicator (⚙ HiPilot / ⚡ Claude has conn), tool name, job status |
...
- [ ] **Status bar** at bottom with current mode
